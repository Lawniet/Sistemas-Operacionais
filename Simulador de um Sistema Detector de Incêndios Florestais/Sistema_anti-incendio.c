...
void *monitoramento(void *param){
    ...
    pthread_t* threads;
    /*Tentativa de obtenção do id para acesso na memória compartilhada*/
    if((idshm1 = shmget(0xFFF, sizeof(itens)*900, IPC_CREAT|0777)) < 0){
        printf("erro na criacao da memória 1: %d\n",idthread);
        exit(1);
    }
    /*Tentativa acesso a memória compartilhada*/
...
/*Função de bombeiro
Parâmetros de entrada: 
    param -> Struct de argumentos com as coordenadas X e Y	
*/
void *bombeiro(void *param){
	ptr_thread_arg targ = (ptr_thread_arg)param;
	int threadid = targ->id, posX = targ->posX, posY = targ->posY;
	int idshm1;
	itens **item;

    /*Tentativa de obetenção do id para acesso na memória compartilhada*/
    if((idshm1 = shmget(0xFFF, sizeof(itens)*900, IPC_CREAT|0777)) < 0){
        printf("erro na criacao da memória!\n");
        exit(1);
    }
    /*Tentativa acesso a memória compartilhada*/
    item = shmat(idshm1,0,0);
	/*Espera de 2s para apagar o fogo*/
	sleep(2);
	item[posX][posY].msg = '*';
...
int main(){
	...
	gera_log("****** Arquivo de log de incendios******\n\n");
    for(int i=0;i<30;i++){
        next_col = 1;
        for(int j=0;j<30;j++){
            if(j == next_col && i == next_lin){
                ...
                /*Trecho de criação das threads de monitoramento*/
				erro = pthread_create(&(threads[cont_id]), NULL, monitoramento, &(arguments[cont_id]));
				if (erro != 0)
            		printf("\nErro ao criar as thread de monitoramento:[%s]\n", strerror(erro));
                next_col += 3;
                cont_id++;
            }
            else{
	...
	if(pid == 0){
    	...
    	while(true){
    		while((lg = msgrcv(msqid,&msg,size_msg,type,IPC_NOWAIT|MSG_NOERROR)) != -1){
				...
				arguments[0].id = id;
				/*Trecho de criação da thread Bombeiro*/
				erro = pthread_create(&(threads_especiais[0]), NULL, bombeiro, &(arguments[0]));
				if (erro != 0)
            		printf("\nErro ao criar as thread de monitoramento:[%s]\n", strerror(erro));
	...
	pthread_join(threads_especiais[0],NULL);
